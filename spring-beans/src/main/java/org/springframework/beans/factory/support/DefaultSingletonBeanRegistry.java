 ...
 *
 * 参考地址:
 * http://www.cnblogs.com/zrtqsk/p/4044687.html
 *
 */
public class DefaultSingletonBeanRegistry extends SimpleAliasRegistry implements SingletonBeanRegistry {
	...
	/** Set of registered singletons, containing the bean names in registration order */ // 存储已经注册的 BeanName
	...
	protected Object getSingleton(String beanName, boolean allowEarlyReference) {
		...
		if (singletonObject == null && isSingletonCurrentlyInCreation(beanName)) {	// 第二个判断, beanName 是否正在处于创建过程中
			synchronized (this.singletonObjects) {
				// 首先检查对应的 bean 是否已经加载过, 因为 singleton 模式其实就是复用以创建的 bean
				// 如果此 Bean 正在加载中则不处理
				singletonObject = this.earlySingletonObjects.get(beanName);
				if (singletonObject == null && allowEarlyReference) {
					// 当某些方法需要 提前初始化的时候 则会调用 addSingletonFactory 方法将对应的 ObjectFactory 初始化策略存储在 singletonFactories
					ObjectFactory<?> singletonFactory = this.singletonFactories.get(beanName);
					if (singletonFactory != null) {
						// 调用预先设定的 getObject 方法
						singletonObject = singletonFactory.getObject();
						// 记录存储在缓存中, earlySingletonObjects 和 singletonFactories 互斥
	...
	public void registerDependentBean(String beanName, String dependentBeanName) {
		...
		// 从容器中: bean 名称 --> 指定名称 bean 的依赖Bean集合找给定名称
		// bean 的依赖 bean
		synchronized (this.dependenciesForBeanMap) {
			...
			// 向容器中: bean 名称 --> 指定 bean 的依赖 Bean 名称集合添加 Bean依赖信息
			// 即, 将 Bean所依赖的 Bean 添加到容器的集合中
