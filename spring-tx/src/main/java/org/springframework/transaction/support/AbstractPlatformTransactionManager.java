...
public abstract class AbstractPlatformTransactionManager implements PlatformTransactionManager, Serializable {
	...
	/**
	 * Set whether existing transactions should be validated before participating (参与)
	 * in them.
	 * <p>When participating in an existing transaction (e.g. with
	 * PROPAGATION_REQUIRES or PROPAGATION_SUPPORTS encountering an existing
	 * transaction), this outer(外部) transaction's characteristics will apply even
	...
	public final TransactionStatus getTransaction(TransactionDefinition definition) throws TransactionException {
		...
		if (definition == null) {
			...
			// 如果没有给定任何事务处理定义就使用 默认定义
			definition = new DefaultTransactionDefinition();
		}

		/**
		 * 判断当前被线程是否存在事务, 判断依据为当前线程记录的连接不为空且连接中(connectionHolder) 中的 tranactionActive 属性不为空
		 * 检查当前线程是否已经存在事务, 如果已经存在事务, 那么需要要根据在事务属性中定义的事务传播属性配置来处理事务的产生
		 */
		if (isExistingTransaction(transaction)) {
			// 这里对当前线程中已经由事务存在的情况进行处理, 结果封装在 TransactionStatus 中
		...
		/**
		 * 如果当前线程不存在事务, 但是 propagationBehavior 却被声明为 PROPAGATION_MANDATORY 抛弃异常
		 *
		 * 当前没有事务存在, 这是需要根据事务属性设置来创建事务
		...
		if (definition.getPropagationBehavior() == TransactionDefinition.PROPAGATION_MANDATORY) {
			...
			try {
				 ...
				 *
				 * 这里是创建事务的调用, 由具体的事务处理器完成, 比如 HibernateTransactioManager
				 * 和 DataSourceTransactionManager 等
				 */
	...
	private TransactionStatus handleExistingTransaction(
		...
		/**
		 * 如果当前已经存在事务, 且当前事务的传播属性设置为 never, 那么抛出异常, 说明是有问题的, Spring 无法处理当前事务的创建
		 */
		...
		if (definition.getPropagationBehavior() == TransactionDefinition.PROPAGATION_NOT_SUPPORTED) {
			...
			// 注意这里的 transaction 为 null, newTransaction 为 false
			boolean newSynchronization = (getTransactionSynchronization() == SYNCHRONIZATION_ALWAYS);
			/**
			 * 意味着事务方法不需要放在事务环境中执行
		...
		/**
		 * 如果当前事务的配置属性是 PROPAGATION_REQUIRES_NEW, 创建新事务, 同时将当前线程存在的事务挂起
		 * 与创建全新事务的过程类是, 区别在于在创建全新事务时不用考虑已有事务的挂起, 但在这里, 需要考虑已有事务的挂起
		 */
	...
	protected final DefaultTransactionStatus prepareTransactionStatus(
		...
		/**
		 * 返回  TransactionStatus 封装事务执行情况
		 * 默认 getTransactionSynchronization = SYNCHRONIZATION_ALWAYS
		 * 所以在这种情况下 newSynchronous 为 true
		 */
		DefaultTransactionStatus status = newTransactionStatus(
	...
	protected DefaultTransactionStatus newTransactionStatus(
			...
			boolean newSynchronization, boolean debug, Object suspendedResources) {
		/**
		 * 这里判断是不是新事务, 如果是 新事务, 那么需要把事务属性存放到当前线程中
		 * TransactionSynchronizationManager 维护一系列的 ThreadLocal 变量来保持
	...
	protected final SuspendedResourcesHolder suspend(Object transaction) throws TransactionException {
		if (TransactionSynchronizationManager.isSynchronizationActive()) {
			...
			try {
				...
				/**
				 * 把挂起事务的处理交给具体的事务处理器完成, 如果具体的事务处理器不支持事务挂起
				 * 那么默认抛出异常TransactionSuspendNotSupportedException
				 */
				if (transaction != null) {
					suspendedResources = doSuspend(transaction);
				}
				// 这里在线程中保存与事务处理有关的信息, 并重置线程中相关的 ThreadLocal 变量
	...
	public final void commit(TransactionStatus status) throws TransactionException {
		...
		if (!shouldCommitOnGlobalRollbackOnly() && defStatus.isGlobalRollbackOnly()) {		// 这里的 shouldCommitOnGlobalRollbackOnly 与 isGlobalRollbackOnly 其实是运用于 分布式事务环境的(比如 JtaTransactionManager), 其中可能还涉及 jndi (PS: 这里其实是比较老的技术, 这个东西的存在也加大了 学习 AbstractPlatformManager 的难度)
			...
			processRollback(defStatus);
			// 抛出 UnexpectedRollbackException 异常
	...
	private void processCommit(DefaultTransactionStatus status) throws TransactionException {
		try {
			...
			try {
				...
				if (status.isNewTransaction() || isFailEarlyOnGlobalRollbackOnly()) {
					globalRollbackOnly = status.isGlobalRollbackOnly(); // 手动设置回回滚 <- 这里是 针对分布式事务
				}
	...
	private void processRollback(DefaultTransactionStatus status) {
		try {
			try {
				triggerBeforeCompletion(status);				// 激活 TransactionSynchronizationManager 中的 TransactionSynchronization 的 beforeCompletion 方法
				if (status.hasSavepoint()) {					// 若事务存在保存点, 则回滚到保存点
					if (status.isDebug()) {
						logger.debug("Rolling back transaction to savepoint");
					}
					status.rollbackToHeldSavepoint();			// 如果有保存点, 则回滚到保存点, 并释放保存点
				} // 当前事务调用方法中新建事务的回滚处理
	...
	private void cleanupAfterCompletion(DefaultTransactionStatus status) {
		// 设置完成状态
		status.setCompleted();							// 设置 TransactionStatus complete = true, 表示事务完成
		if (status.isNewSynchronization()) {
			TransactionSynchronizationManager.clear();  // 清空 TransactionSynchronizationManager 中的设置信息
		}
		if (status.isNewTransaction()) {				// 释放连接池
			doCleanupAfterCompletion(status.getTransaction());
		}
		if (status.getSuspendedResources() != null) {	// 若存在挂起来的事务, 则恢复
			if (status.isDebug()) {
